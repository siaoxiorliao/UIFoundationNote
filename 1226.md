...
> * Create groups 创建组(自己手动创建的组(黄色文件夹)的文件实际上不会被创建进项目中,但从外部引入的则会进入项目中),创建的是黄色文件夹,这文件是会被编译的,使用的时候则是这样#include xxx.h
> * Create folder references 创建引用,这些文件不会被编译,创建的是蓝色文件,使用的时候则是这样#include myScripts/xxx.h
